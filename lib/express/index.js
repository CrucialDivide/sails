module.exports = function (sails) {
	...
	return function loadExpress (cb) {
		...
		/////////////////////////////////////////////////////////////////
		//if (sails.config.controllers.subdomains) {
			var subdomains = require("express-subdomains");
			subdomains.domain("");
			//subdomains.domain(sails.config.domain);
			var domains = sails.config.subdomains;
			util.each(domains,
				function(domain){
					//console.log("subdomains.use", domain);
					this.use(domain.prefix, {pathMatch: domain.pathMatch});
				},
				subdomains
			)
			subdomains.init();

			//('hook:controllers:bound:action', actionId, controllerId, blueprintsPrefix, indexRoute);
			sails.on('hook:controllers:bound:action',
				function(actionId, controllerId, blueprintsPrefix, route) {
					util.each(domains,
						function(domain){
							if (domain !== '') {
								sails.log.warn("Domain Binding", domain.prefix, route, controllerId, actionId);
								sails.router.bind("/"+domain.prefix+route, _serveBlueprint(controllerId, actionId));
							}
						}
					)
				}
		...
		sails.on('router:bind', function (route) {
			...
			route = util.clone(route); //console.log("express:index.js - on router:bind", route)
		...
		// TODO: 400 (do we need referrer for non-JSON requests?)
		// TODO: 403/401 (do we need referrer for non-JSON requests?)
