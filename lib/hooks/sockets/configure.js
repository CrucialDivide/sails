module.exports = function (sails) {
	...
	return function configure (cb) {
		...
		SocketServer.listen(sails.express.server, {
			logger: {
				info: function (){}
			}
		});
		...
		util.each(socketConfig, function(value, propertyName) {
			...
			if (propertyName === 'adapter' && value === 'redis') {
				...
				var pub = createRedisConnection(port, host, socketConfig);
				var sub = createRedisConnection(port, host, socketConfig);
				var client = createRedisConnection(port, host, socketConfig);
		...
		var channels = socketConfig.channels || []; console.log("channels", channels, socketConfig);
		if (!sails.util._.isArray(channels)) {
			channels = [channels];
		}
		// Link Socket.io requests to a controller/action
		// When a socket.io client connects, listen for the actions in the routing table
		...
		sails.util._.each(channels,
			function(channel) {
				sails.log.verbose("Connecting Channel: ", channel)
				io.of(channel).on('connection', Socket.connection)
			}
		);
	...
	function createRedisConnection(port, host, socketConfig) {
