...
module.exports = function enhanceRequest(req, res, controllerName, actionName) {
	...
	res.view = function(specifiedPath, data, fun) {
        ...
        sails.log.verbose("request:view rendering: "+renderNumericStatusAsFriendly(path));
    ...
    function renderNumericStatusAsFriendly(code) {
        var http = require("http");

        if (_.isNumber(code) || !_.isNaN(_.parseInt(code))) {
            return http.STATUS_CODES[_.parseInt(code)]+" ("+code+")";
        } else {
            return code;
        }
    }
