...
/**
 * Repeatable blueprint API functionality
 */
var controllerBlueprint = require('../blueprints/controller');
var notFoundBlueprint = require('../blueprints/notFound');

/**
 * Route request through policies
 */
var policyRouter = require('./policies');
...
function fetchRoute(url, verb) {
	...
	// Target is a string which maps to a controller/action which may not exist yet
		...
		if(target.match('/')) {
			...
			routeObj = sails.util.parsePath(target);
			...
			routeObj = sails.util.parseStringRoute(target);
...
function guessAction(req) {
	...
	// If controller is KNOWN
	// But requestedAction is unknown in controller
	// AND not a CRUD action (TODO: remove this part)
	// then treat requestedAction as id
	else if( controller && !controller[requestedAction] && !isCrudAction(requestedAction) && requestedAction !== 'index') {
...
function handler(req, res, next) {
	...
	policyRouter(requestedEntity, actionName, req, res, function afterwards(err) {

		// Run default Error Handler if an error is detected.
		// Allows next(err) to be called in policy middleware.
		...
		function afterThat(err, exists, viewPath) {
			...
			if (qualifiedForBlueprint(requestedEntity, actionName)) {
				var nextFn = controllerBlueprint.definition(requestedEntity)[actionName];
			...
			// If it's not valid
				...
				if(qualifiedForBlueprint(requestedEntity, actionName)) {
					...
					var blueprint = controllerBlueprint.definition(requestedEntity);
					var action = blueprint[actionName];
					return runHandler(req, res, next, action);
				...
				// Otherwise if the proper view exists, go ahead and render it
                    ...
                    sails.log.verbose("viewPath: "+viewPath);
					res.view(viewPath);
				});
