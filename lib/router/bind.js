module.exports = function (sails) {
	...
	function bind ( path, target, verb ) {
		//sails.log.warn("bind:bind", path, target, verb);
		...
		if (util.isObject(target) && !util.isFunction(target) && !util.isArray(target)) {
			var _ = sails.util._;
			if (_.has(target, "forDomain")) {
				var subDomain = target.forDomain;
				delete target.forDomain;
				sails.log.warn("Matched forDomain", subDomain);
				_.each(target,
					function(subTarget,subPath){
						bind("/"+subDomain+subPath, subTarget, verb);
					}
				)
			}
