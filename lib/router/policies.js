...
function runPolicy(controllerName, actionName, req, res, next) {
	...
	if (!_.isUndefined(cPolicy)) {
		routePlan = cPolicy;
        sails.log.verbose("routePlan: "+JSON.stringify(routePlan));
...
function buildPolicyChain(policies, req, res, afterwards) {
	...
	for (var i=policies.length-1; i >= 0; i--) {
		...
		var policySet = interpretPolicy( policies[i] );

        // Handle arrays - convert to an array if the result of interpretPolicy is simple
        if (policySet && !_.isArray(policySet)) {
            policySet = [policySet];
        }
		// Set up chain to call next middleware after itself
        for (var j=policySet.length-1; j >= 0; j--) {
            chain = _.bind(policySet[j], {}, req, res, chain);
        }
	}
